...
$(function() {
	...
	$('.check_city ul li').click(function() {
		...
		switch(action) {
			case "buy":
				dic.goodsType(cityId);
				break;
		}

		$('.locacity span').html(cityName);
		$('.locacity span').attr('data-name', cityName);
		$('.city_list .cur_city').html(cityName);
		$('.city_list .cur_city').attr('data-name', cityName);
		//$(this).parents('.city-lists').find('a').removeClass('selected');
	...
	$('.isprice li').on("click", function() {
		...
		if(index == 0) {
			...
			$('#startDate').attr('req', 'false');
		} else {
			$('#money').attr('req', 'false');
			$('#yueDate').attr('req', 'true');
			$('#startDate').attr('req', 'true');
		}
...
function drawMyMapPath(start, end) {
	//起始点坐标
	var startIcon = new BMap.Icon("images/start.png", new BMap.Size(18, 32), {
		offset: new BMap.Size(10, -25) // 指定定位位置  
			//imageOffset: new BMap.Size(0, 0 - 10) // 设置图片偏移 
	});
	//结束点坐标
	var endIcon = new BMap.Icon("images/end.png", new BMap.Size(18, 32), {
		offset: new BMap.Size(10, -25) // 指定定位位置  
			//imageOffset: new BMap.Size(0, 0 - 10) // 设置图片偏移 
	});
