...
import { ENVIRONMENT } from '@core/token';
...
export class UserLoginComponent implements OnDestroy, OnInit {
    ...
    submit() {
        ...
        setTimeout(() => {
            this.loading = false;
            // login
            const url = this.util.getWebUrl('open', { open: 'we7/login' });
            this.http.post(url, this.form.value).subscribe((res: any) => {
                if (res.return_code === 'ok') {
                    const record = res.return_content;
                    ...
                    console.log('登录成功');
                    this.msg.success(res.return_msg);
                    this.router.navigate(['/web/site/entry/', this.env.m, this.env.do]);
                } else {
                    console.log('登录失败');
